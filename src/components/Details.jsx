...
function Details() {
	...
	return (
		<section className='flex items-center justify-center bg-light-grayish-cyan-bg'>
			<article className='container mx-auto flex max-w-2xl flex-col items-center gap-6 rounded-lg bg-white p-6 text-center'>
				...
				<div>
					<h2 className='text-2xl font-bold text-slate-800'>{job?.position}</h2>
					<p className='my-5 text-lg'>{job?.description}</p>
				</div>

				<div className='mb-4 flex flex-wrap gap-3 lg:mr-5 lg:self-center'>
					{tags.map(tag => (
						<span className='rounded-sm bg-light-grayish-cyan-ft px-2 py-1 pb-0 text-base font-bold text-dark-cyan'>
							{tag}
						</span>
					))}
				</div>
